...
- **Distributed Query Execution**: Scatters queries to the relevant Data Nodes in parallel via an internal binary NIO protocol (with Snappy compression and AES encryption), then merges partial results (sort-merge, aggregation, LIMIT). Work that cannot be answered shard-locally is finished here rather than pushed down: `DISTINCT` and the set operators de-duplicate across shards, `COUNT(DISTINCT …)` merges the shards' value sets, window frames span rows no single shard holds, subquery predicates run their inner query first, and `TABLESAMPLE` draws its sample from the gathered rows. For each of those the `LIMIT` pushdown is withheld, because a shard that returned only `limit` rows would change the answer.
- **Distributed Transactions**: Supports ACID transactions across shards using a two-phase commit protocol (PREPARE + COMMIT).
- **Cluster Metadata Management**: The leader Coordinator maintains table schemas and shard maps in an encrypted RocksDB-backed metadata store, and synchronizes them to non-leader Coordinators — pushing a snapshot on every change, and each follower re-pulling on an interval (`neorunbase.coordinator.metadata.refresh.interval.ms`) so a push that never landed cannot leave a follower answering "table not found" for a table that exists.
